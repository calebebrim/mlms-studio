...
% n (1 × 1)      : (optional)
%                  n = integer n.: maximum shift correction in data points
...
if ~nargin
    ...
    fprintf('\n\n icoshift(target,dataset,alignment mode,No. points of shift,plot options);')
    fprintf('\n or')
    fprintf('\n icoshift(target,dataset,alignment mode);\n')
    fprintf('\n Examples:')
    fprintf('\n icoshift(''average'',xP,20,[],2);  splits in 20 regular intervals and switch off plots and warnings\n')
    fprintf('\n icoshift(target,xP,(1000:2000));  aligns the whole spectra according to the reference signal in (1000:2000)\n')
...
%% Check input GT # Reorganised slightly to have distinct blocks for each input parameter
% xT and xP
max_flag = 0;
...
if flag_coshift
   ...
   [xP,nil,wint] = icoshift(xT,xP,'whole',n_co,[0 1 0]); %FRSA preprocessing COhift doesn't use NaNs
...
if nint ~= 1, error('"inter" has to be a row vector and not a matrix'); end
if ~any(strcmpi(n,{'f','b'})) && numel(inter)~= mint
         if any(diff((reshape(inter,2,mint/2)),1,1) < n)
...
if ischar(n) && (numel(n) > 1 || ~any(ismember('bf',lower(n))))
    error('"n" must be a scalar/''f''/''n''')
elseif isa(n,'numeric')

    if any(n <= 0),  error('ERROR: shift(s) "n" must be larger than zero'); end
    if numel(n) > 1, wmsg = sprintf('"n" must be a scalar/character; first element (i.e. %i) used',fix(n)); end
    if n ~= fix(n),  wmsg = sprintf('"n" must be an integer; first element (i.e. %i) used',fix(n)); end
    n = round(n(1));
    if ~isempty(wmsg)
...
if length(Scal) ~= mP, error('X and Scal are not of compatible length'), end
flag_ScalDir = Scal(1) < Scal(2);
if length(unique(sign(diff(Scal)))) ~= 1, error('Scal must be strictly monotonic'); end
...
[mi,pmi] = min(inter);
[ma,pma] = max(inter);
flag2    = length(inter) > 1 && isequal([1,pma - pmi + 1],size(inter)) && isequal(unique(diff(inter,1,2)),1);
if any(vec(flag))
...
if  flag % a Reference signal has been chosen
    ...
    if max_flag
        [amax,bmax] = max(sum(xP(:,inter),2)); % MAX
        xT  = xP(bmax,:);                      % MAX
    end
    ind          = NaN(nP,1);
    [xCS,ind(:)] = coshifta(xT,xP,inter,n,options);
    ints         = [1 mi ma];
    ...
    if mint > 1
        ...
        if max(inter)>mP, error('Intervals ("inter") exceed samples matrix dimension'); end
    ...
    for i = 1:size(allint,1)
        ...
        if ~all(isnan(target)) && sum(missind) ~= 0
            [cosh_interval,loc_ind]              = coshifta(target,intervalnow(missind,:),0,n,options);
...
if auto == 1
    ...
    while auto == 1 % Automatic search for the best "n" for each interval
        ...
        [dummy,ind,R] = CC_FFTShift(xT(:,refW),xP(:,refW),[-n n 2 1 Filling]); %FFT Co-Shifting
        temp_index = -n:n;
        for i_sam = 1:nP
    ...
    if rw == 1, refW = 1:mP; end
    [dummy,ind,R] = CC_FFTShift(xT(:,refW),xP(:,refW),[-n n 2 1 Filling]);
    temp_index = -n:n;
    for i_sam = 1:nP
        index       = find(temp_index == ind(i_sam));
        xW(i_sam,:) = xtemp(i_sam,index:index + mP - 1);
...
X_fft      = full(X_fft * inv(sparse(1:prod(dimX(ord(2:end))),1:prod(dimX(ord(2:end))),sqrt(SumwNaN(X_fft.^2)))));
T          = permute(T,ord);
...
% Last modified: 10 March, 2009; 16:18
...
if isempty(flag) || (numel(flag) == 1 && flag)
    flag = isnan(X);
    N    = any(flag(:));
elseif isequal(size(flag),size(X)), N = any(flag(:));
