...
for ii=1:cicles
    ...
    [x, fval, ~, ~, xpop] = gamultiobj(@multiobj,sins*3,[],[],[],[],[],[],gaopt);
...
feature.transform.name = 'fn_sum_sins(params, data)';
    ...
    function mret = multiobj(arg)
        mret(1) = fitness_fr(arg,wtr,ctr);
        mret(2) = fitness_counting(arg,wtr,ctr);
    end
    

    function state = plot_scores(options,state,flag)
        
        axx = findobj('Tag','ga_fr/plot_scores');
...
%                             plot_sins(state.Population(best,:),wtr,ctr,axx);
                            ...
                            plot(state.Score(:,1),state.Score(:,2),'.b');
                            hold on;
                            plot(state.Score(best,1),state.Score(best,2),'or');
                            
%                             plot([0 size(state.Score,1)],repmat(mean(state.Score(best,2)),1,2),'-r')
                            hold off;
                        end
                end
            end
